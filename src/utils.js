...
export function randomColor() {
	...
	color.s = getRandomInt(80, 90);
	color.l = getRandomInt(60, 70);
	return color;
}
...
export function getContrastingColor(color) {
	...
	// Counting the perceptive luminanceValue - human eye favors green color...
	const luminanceValue =
		(0.299 * color.r + 0.587 * color.g + 0.114 * color.b) / 255;

	if (luminanceValue > 0.5) d = 31;
...
export function getCss(style, theme, outline) {
	let backgroundColor,
		color = theme[style].hex;
	let oppositeColor;
	if (style === 'primary') {
		oppositeColor = 'secondary';
	} else {
		oppositeColor = style === 'accent' ? 'accent' : 'primary';
	}
	...
	if (style) {
		...
		if (outline) {
			if (style === oppositeColor) {
				color = getContrastingColor(theme[oppositeColor]).hex;
			} else {
				color = getContrastingColor(theme[style]).hex;
			}
		}
	}
